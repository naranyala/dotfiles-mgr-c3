...
export function render() {
	const presetBtns = PRESETS.map(p =>
		`<button data-run-cmd="${p.cmd}" class="btn-icon" style="font-size:0.78rem;padding:5px 10px;white-space:nowrap">${p.icon} ${p.label}</button>`
	).join('')

	const historyItems = state.history.length
		? state.history.map(h =>
			`<span data-run-cmd="${h.cmd}" style="cursor:pointer;font-size:0.72rem;color:#64748b;padding:2px 6px;background:rgba(255,255,255,0.03);border-radius:4px;white-space:nowrap" title="${h.cmd}">${h.cmd.length > 20 ? h.cmd.substring(0, 20) + '…' : h.cmd}</span>`
		).join(' ')
		: ''
